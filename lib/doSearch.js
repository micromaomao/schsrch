  ...
  function doSearch (query) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if ((match = lQuery.match(/^!!index!([0-9a-f]+)$/))) {
        ...
        let queTokenized = IndexContent.lcsRemoveSpace(IndexContent.tokenize(query))
        const lcsWordsLimit = 60
        if (queTokenized.length > lcsWordsLimit) {
          queTokenized = queTokenized.slice(0, lcsWordsLimit)
        }
        PastPaperIndex.search(query).then(results => {
          Promise.all(results.map(rst => new Promise((resolve, reject) => {
            ...
            response: 'text',
            list: rst.map(a => {
              let cont = a.index.content
              return Object.assign(a, {lcsScore: IndexContent.lcsLength(IndexContent.tokenize(cont), queTokenized)})
            }).sort((a, b) => {
              return Math.sign(b.lcsScore - a.lcsScore)
            })
          }), err => reject({response: 'error', err: err.toString()}))
        }).catch(err => {
      ...
      function fetchPP (subject, time, paper, variant, type) {
        ...
        PastPaperDoc.find(finder, {fileBlob: false}).limit(51).then(rst => {
          if (rst.length >= 50) {
            resolve({
              response: 'overflow'
            })
