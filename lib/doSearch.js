        ...
        PastPaperIndex.search(query).then(results => {
          Promise.all(results.map(rst => new Promise((resolve, reject) => {
            ...
            list: rst.sort((a, b) => {
              ...
              return Math.sign(IndexContent.lcsLength(IndexContent.tokenize(aContent), queTokenized) - IndexContent.lcsLength(IndexContent.tokenize(bContent), queTokenized))
            })
          }), err => reject({response: 'error', err: err.toString()}))
        }).catch(err => {
