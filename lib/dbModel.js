...
module.exports = (db, es) => {
  ...
  let idSchema = new mongoose.Schema({
    authToken: {type: mongoose.Schema.Types.Buffer, index: true, required: true}, // A 16 byte token that needs to be sent to the server with every private API request.
    creationTime: {type: 'Number', index: true, required: true},
