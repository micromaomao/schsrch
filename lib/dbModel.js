...
module.exports = (db, mongoose) => {
  let docSchema = new mongoose.Schema({
    ...
    dir: {type: [new mongoose.Schema({
      ...
      qNRect: {type: 'Object', required: false, default: null}
    })], default: [], required: false}
  })
  let indexSchema = new mongoose.Schema({
    doc: {type: 'ObjectId', required: true, index: true},
    page: {type: 'Number', required: true, index: true}, // starts from 0
  ...
  docSchema.method('ensureDir', function () {
    if (!this.doc) throw new Error('this.doc must present.')
    return new Promise((resolve, reject) => {
      if (this.dir && this.dir.length > 0) return resolve(this.dir)
      PastPaperIndex.find({doc: this._id}).sort({page: 1}).exec().then(idxes => {
        return Promise.all(idxes.map(idx =>
          new Promise((resolve, reject) => {
            sspdf.getPage(this.doc, idx.page, function (err, pageData) {
  ...
  indexSchema.index({doc: true, page: true})
  try {
