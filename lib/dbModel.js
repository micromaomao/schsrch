...
module.exports = (db, es) => {
  ...
  let docSchema = new mongoose.Schema({
    ...
    fileBlob: {type: mongoose.Schema.Types.Buffer, required: true},
  ...
  let collectionSchema = new mongoose.Schema({
    ...
    allowedRead: {type: [mongoose.Schema.Types.ObjectId], required: false, default: []},
    allowedWrite: {type: [mongoose.Schema.Types.ObjectId], required: false, default: []}
  })

  let idSchema = new mongoose.Schema({
    authToken: {type: mongoose.Schema.Types.Buffer, required: true}, // A 16 byte token that needs to be sent to the server with every private API request.
    creationTime: {type: 'Number', index: true, required: true},
    lastSuccessLoginIp: {type: 'String', required: false, default: null},
    lastSuccessLoginTime: {type: 'Number', required: false, default: null},
    username: {type: 'String', required: true}
  })
  idSchema.index({username: 1}, {unique: true})
  idSchema.index({authToken: 1}, {unique: true})
  ...
  return Promise.all(awaiting).then(() => Promise.resolve({PastPaperDoc, PastPaperIndex, PastPaperFeedback, PastPaperRequestRecord, PastPaperCollection, PastPaperId}))
}
