...
module.exports = (db, es) => {
  ...
  let indexSchema = new mongoose.Schema({
    docId: {type: 'ObjectId', required: true, index: true},
  ...
  indexSchema.method('indexToElastic', function (content, doc) {
    ...
    let gotDocs = docs => {
      if (docs.length === 0) return Promise.reject(new Error('Corrosponding doc not find.'))
      let doc = docs[0]
      let esDoc = {
    ...
    return PastPaperDoc.find({_id: this.docId}, {fileBlob: false}).then(gotDocs)
  })
