module.exports = (db, mongoose) => {
  ...
  indexSchema.static('search', query => new Promise((resolve, reject) => {
    ...
    aggreArray.push({$project: {tDoc: {$arrayElemAt: ['$doc', 0]}, _id: true, doc: {$literal: "Document content not sent."}, content: true, page: true}})
    aggreArray.push({$addFields: {'tDoc.doc': {$literal: "Document content not sent."}}})
    if (queryPerfixMatch) {
    ...
    PastPaperIndex.aggregate(aggreArray).then(res => {
      resolve(res.map(mgDoc => (mgDoc.tDoc._id ? {
        doc: Object.assign(mgDoc.tDoc, {doc: null}),
        index: {
          _id: mgDoc._id,
