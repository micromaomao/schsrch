...
module.exports = (db, es) => {
  ...
  PastPaperDoc.on('index', handleIndex)
  PastPaperIndex.on('index', handleIndex)
  ...
  PastPaperCollection = db.model('pastPaperCollection', collectionSchema)
  return Promise.all(awaiting).then(() => Promise.resolve({PastPaperDoc, PastPaperIndex, PastPaperFeedback, PastPaperRequestRecord, PastPaperCollection}))
}
