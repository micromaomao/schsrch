...
module.exports = {
  ...
  regionBlank: function (rTree, region) {
    ...
    if (!rects.every(r => /^\s$/.test(r.t))) { // `every` returns true for []
      return false
    }
    return true
  },
      ...
      idxes.slice(fromII).forEach((idx, ii) => {
        ...
        for (let lastIndex = 0; (lastIndex = sortedContent.indexOf(search, lastIndex)) >= 0; lastIndex += search.length) {
          ...
          if (frontRect.x1 < xLimit && this.noRectsLeftOf(rTree, qRect)
            && (
                ...
                (() => {
                  ...
                  return this.regionBlank(rTree, {
                    minX: Math.max(lastCharRect.x1, lastCharRect.x2),
            ...
            xLimit = Math.min(xLimit, frontRect.x1 + Math.abs(frontRect.x2 - frontRect.x1) * 3)
            nextQuestionNum ++
